...
previous=json.loads((OUT/'fit-support-fk-result.json').read_text())['samples']
rows=[]
for t in [.8,1.7]:
 ...
 def stats(d,q):
  ...
  joint=[]
  for j in range(2):
   cr=axes[j].cross(axes[j+1]);sg=1 if cr.dot(C.to_3x3()@Mw.to_3x3()@rest['wrist.R'].to_3x3().inverted()@Vector((-1,0,0)))>=0 else -1
 ...
 cached=next(r for r in previous if r['t']==t)
 if t==.8:
  cached['finger_angles']['4']=[-.027,-.076,-.093];cached['finger_angles']['5']=[-.205,.005,.01]
 else:
  cached['finger_angles']['3']=[-.055,-.10,-.13];cached['finger_angles']['4']=[-.0345,-.10,-.09];cached['finger_angles']['5']=[-.1405,-.10,.01]
 x=np.array(sum([cached['finger_angles'][str(d)] for d in range(2,6)],[])+cached['additional_cover_translation_yz']+[cached['spreads']['2'],cached['spreads']['3']])
 best=score(x)
 for step in []:
 ...
 for d in range(2,6):
  ...
  rr['digits'][str(d)]['actual_skin_bounds']=[[float(actual[:,j].min()),float(actual[:,j].max())] for j in range(3)]
  rr['digits'][str(d)]['actual_skin_highest_point']=actual[np.argmax(actual[:,2])].tolist()
 ...
 for add in [-.60,-.70,-.80]:
  for opp in [0,.04]:
   q=[-.30,-.05,-.03];tip,axis,vv,_,ang=calc(1,q,opp,add);vv[:,1:]+=x[12:14];tip[1:]+=x[12:14]
 ...
 dz=.1595-max(rr['digits'][str(d)]['actual_skin_max_z'] for d in [2,3])
 dy=-.483-np.mean([rr['digits'][str(d)]['actual_skin_pad_top_centroid'][1] for d in [2,3]])
 rr['final_contact_translation_correction_yz']=[float(dy),float(dz)]
 rr['bone_tip_mean_target'][1]+=float(dy);rr['bone_tip_mean_target'][2]+=float(dz)
 for d in range(2,6):
  pt=rr['digits'][str(d)]['actual_skin_pad_top_centroid'];pt[1]+=float(dy);pt[2]+=float(dz)
  rr['digits'][str(d)]['final_skin_top_gap_to_underside_author_m']=float(.1595-(rr['digits'][str(d)]['actual_skin_max_z']+dz))
 print('BLENDER_SKIN_ERROR',errors)
 rows.append(rr)
(OUT/'fit-support-fk-final.json').write_text(json.dumps({'sha':sha,'samples':rows},indent=2))
