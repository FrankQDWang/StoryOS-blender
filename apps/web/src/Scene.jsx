...
function MagicTable({onCreate,crafting,reduced}){
 ...
 return <group position={TABLE} onClick={e=>{e.stopPropagation();onCreate()}} onPointerOver={()=>document.body.style.cursor='pointer'} onPointerOut={()=>document.body.style.cursor=''}>
  ...
  {/* Decorative points must not intercept books behind the table. */}
  {!reduced&&<Sparkles raycast={()=>null} count={crafting?100:18} scale={[1.15,crafting?2.8:.65,1.15]} size={crafting?4:2} speed={crafting?1.5:.18} color="#c6dfb2" opacity={.5}/>}
