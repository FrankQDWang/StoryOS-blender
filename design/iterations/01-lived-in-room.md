...
状态：历史方案。2026-09-14 用户实际体验后认为不如第一版，要求回滚，仅保留壁炉结构优化思路。下文是当时的需求、提案和结果，不再是当前实施指令；最新决定见 `02-baseline-hearth.md`，当时验收见 `../../evidence/v02/design-qa.md`。
...
已确认：是生活感类比，目标让人放松，不需要床。用户允许在当前基线上继续下一版；仅当有影响方向的新问题时再问。

## 版本边界

`v0.1.0-baseline` 与 `main` 保留。用户随后明确生活感类比并授权继续下一版，现已在 `iteration/next` 实现。`v0.2.0-preview.1` 是待评审快照，不能视为用户已认可新美术。

## v0.2 执行方案

- 收紧房间尺度，窗、桌、书柜不再构成中轴；矮书柜承载五个稳定位置，书竖放，保留现有 slot 数据。
- 窗边书桌提供魔法新建与固定开书位置；书从柜中短距离移动到桌面，双手与书共用动画时序。
- 连续砌筑的壁炉、附近软椅和少量布料/木柴；局部物件按使用关系聚集，通道清楚。
- 复用已认可的木/灰泥纹理、夜间冷暖光与 HUD，制作和检查真实 3D，不把生成图当成背景。
- v0.1.0-baseline 原样保留；下一版在 iteration/next 上作为待用户评审的预览，不自动覆盖已认可版本。

## 实现结果

上述执行方案已落实到可运行 3D。新全景：

![v0.2 实际 Chrome 全景](../../evidence/v02/overview.png)

壁炉的连续拱口、软椅与搭布、矮柜、窗边书桌均为真实几何。开书在桌面完成，右手随封面铰链，左手稍后扶住纸页；桌灯避开展开范围。细节仍是样片精度，可继续按用户反馈调整。
