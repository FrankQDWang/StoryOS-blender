...
| `main` | 用户认可的当前版本；2026-09-17已将0.1.4-preview.4经PR #2合入本地及远端同步主分支，成果提交e6d033a、合并提交fcaa7d1 |
| 迭代分支 | 所有迭代均已合入并删除，包括codex/wrist-motion-step；本地和远端只保留main，未来按具体任务新建codex/前缀分支 |
...
当前 `4173` 为基于已保存 `0.1.4-preview.2` 的人体尺寸与侧边夹持改进 `0.1.4-preview.4`，已获用户认可并通过PR #2保存到同步的main；本地与远端迭代分支均已清理，交接见PLAN。需要对照时，`4175` 使用固定的 `v0.1.0-baseline` worktree。两个 worktree 分别安装本地依赖；不共享 `node_modules/.vite` 缓存，以免两个开发服务反复覆盖依赖预构建。两处预览均是本机服务。
