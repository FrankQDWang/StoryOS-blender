...
| `iteration/next` | 当前保存用户已认可的 `0.1.2-preview.4` 暖炉与阅读台布局；继续用新提交保存进度 |
...
4. 用户认可后按当次授权保存；合入 `main` 与新增里程碑标签需有对应发布决定。本次 2026-09-16 仅提交并 push `iteration/next`，不移动 `main` 或已有标签。
...
当前 `4173` 为用户已认可的 `0.1.2-preview.4` 暖炉与阅读台布局。需要对照时，`4175` 使用固定的 `v0.1.0-baseline` worktree。两个 worktree 分别安装本地依赖；不共享 `node_modules/.vite` 缓存，以免两个开发服务反复覆盖依赖预构建。两处预览均是本机服务。
